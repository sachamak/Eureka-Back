...
export const shouldSkipComparison = (
  lostItem: IItem,
  foundItem: IItem
): boolean => {
  if (lostItem.isResolved || foundItem.isResolved) return true;

  if (!foundItem.date || !lostItem.date) {
    return true;
  }

  if (new Date(foundItem.date) < new Date(lostItem.date)) {
    return true;
  }
...
export const MatchingService = async (
  targetItem: IItem,
